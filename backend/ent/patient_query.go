...
func (pq *PatientQuery) QueryPillorders() *PillorderQuery {
	query := &PillorderQuery{config: pq.config}
...
func (pq *PatientQuery) WithPillorders(opts ...func(*PillorderQuery)) *PatientQuery {
	query := &PillorderQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withPillorders = query
	return pq
}
...
func (pq *PatientQuery) sqlAll(ctx context.Context) ([]*Patient, error) {
	...
	if query := pq.withPillorders; query != nil {
		...
		query.Where(predicate.Pillorder(func(s *sql.Selector) {
			s.Where(sql.InValues(patient.PillordersColumn, fks...))
		}))
