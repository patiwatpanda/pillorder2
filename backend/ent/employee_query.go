...
func (eq *EmployeeQuery) QueryPillorders() *PillorderQuery {
	query := &PillorderQuery{config: eq.config}
...
func (eq *EmployeeQuery) WithPillorders(opts ...func(*PillorderQuery)) *EmployeeQuery {
	query := &PillorderQuery{config: eq.config}
	for _, opt := range opts {
		opt(query)
	}
	eq.withPillorders = query
	return eq
}
...
func (eq *EmployeeQuery) sqlAll(ctx context.Context) ([]*Employee, error) {
	...
	if query := eq.withPillorders; query != nil {
		...
		query.Where(predicate.Pillorder(func(s *sql.Selector) {
			s.Where(sql.InValues(employee.PillordersColumn, fks...))
		}))
